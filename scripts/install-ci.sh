...
install_metadata="${SITES_PROJECT_ROOT}/node_modules/.sites-install.json"
vinext="${SITES_PROJECT_ROOT}/node_modules/.bin/vinext"
if [[ -x "${vinext}" && -f "${install_metadata}" ]]; then
  if node --input-type=module - "${install_metadata}" "${lockfile_sha256}" <<'NODE'
import { readFile } from "node:fs/promises";

const metadata = JSON.parse(await readFile(process.argv[2], "utf8"));
const expectedPlatform = `${process.platform}-${process.arch}`;
if (
  metadata.lockfile_sha256 !== process.argv[3] ||
...
NODE
  then
    echo "[sites] installed dependencies match the lockfile; reusing node_modules"
    exit 0
  fi
fi
